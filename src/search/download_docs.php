...
if (isset($_POST['download_id'])) {
    ...
    if ($dissertation_id <= 9999)
    {
        // The path where your dissertation directory will be.
        $path = DISSERTATIONS_PATH . "000/" . $dissertation_id . "/";
    }

    else {
        $num = $dissertation_id / 10000;
        $uppersubdir = sprintf('%03d',floor($num));
        $lowersubdir = substr(strval(abs($uppersubdir - $num)), 2);

        $path = DISSERTATIONS_PATH . $uppersubdir . '/' . $lowersubdir . '/';
    }
